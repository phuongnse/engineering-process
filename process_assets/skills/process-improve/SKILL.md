...
`[consumer-process][CONSUMER-KEY][PROCESS-VERSION][INVARIANT][INCIDENT-KIND]` across open
...
Use the stable title key `[consumer-process][CONSUMER-KEY][PROCESS-VERSION][INVARIANT][INCIDENT-KIND]`. The `CONSUMER-KEY` segment is generated by the runtime key builder: ASCII letters, digits, dots, and hyphens stay unchanged; every other UTF-8 byte becomes `_hh` (for example, `phuongnse/lyric-rail` becomes `phuongnse_2flyric-rail`). Search before creating:
