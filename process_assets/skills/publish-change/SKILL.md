---
name: publish-change
description: Publish a completed, independently reviewed checkpoint as a branch, pull request, or equivalent review object, then stop for human merge authorization. Use after lifecycle completion when project policy authorizes publication automation.
---
...
5. Stop at merge. Only the configured human owner authorizes merge; automation must
   not enable automerge or invoke a merge operation.
...
- Do not create a review object for a merely planned, implementing, verified,
  review-pending, changes-requested, or approved lifecycle.
- Do not treat static policy verification as semantic independent review.
- Do not merge on behalf of the human owner.
