...
SOURCE_REF = re.compile(
    r"^refs/tags/epv/(?P<change>[0-9a-f]{16})/"
    r"c(?P<cycle>[1-9][0-9]{0,6})/"
    r"(?P<base>[0-9a-f]{16})/(?P<head>[0-9a-f]{40})/"
...
def validate_dispatch(
    ...
    *,
    ...
    if re.fullmatch(r"[a-z0-9](?:[a-z0-9-]*[a-z0-9])?", change_id) is None:
        raise ValueError("change id is invalid")
    change_digest = hashlib.sha256(change_id.encode("utf-8")).hexdigest()[:16]
    for value, label in (
    ...
    if (
        match.group("change") != change_digest
        or match.group("base") != comparison_base[:16]
